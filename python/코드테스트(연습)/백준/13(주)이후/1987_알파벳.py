...
Y,X = map(int, input().split()) # 세로축 높이, 가로축 길이
board = [] # 알파벳 보드 
alphabet = set() # 최대 칸수를 세기 위해 (시간 줄이기 위해)
for y in range(Y):
    data = input()
    for i in data:
        alphabet.add(i)
...
max_ = 0 # 최대 칸수
while stack:
    stack_instance = stack.pop()
    cnt = stack_instance[2]
    while len(visited) >= cnt: # 탐색하다 길 못찾아 백하면 그에 맞게 visited 리셋
        visited.pop()
    visited.append(stack_instance[0]) # 내가 간 길에 순서에 맞게 기록이 되어 있습니다. 
    ...
    cnt += 1
    for i in range(4):
        ny = stack_instance[1][0] + dy[i]
        nx = stack_instance[1][1] + dx[i]
